...
import { sendEmail } from "../services/emailapp.service.js";
import { otpEmailTemplate } from "../utils/otphtml.utils.js";
...
export async function register(req, res) {
  ...
  // pehle hm gmail verify krenge then hi koi step lenge 
  // access and refresh token hm login ke time gen krenge not this time
  // yha bs email verify hogi

  // gen otp and send otp
  ...
  //  send email
  const otphtml = otpEmailTemplate(otp);
...
if (!mailSent) {
  ...
  const user_otp = await otpmodel.create({
    email,
    user:user._id,
    otp:otphash,
...
export async function login(req,res){
    ...
    // if user verified nhi h to pehle verify krke aa bete
    if(!user.verified){
      res.status(400).json({
        message:"plz verify for login"
      })
    }
...
export async function verifyotp(req,res){
  ...
  //  make the user verifiable 
   ...
   await otpmodel.deleteOne({ _id: user._id });
   res.status(200).json({
